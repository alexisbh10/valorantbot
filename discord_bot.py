...
async def _descargar_iconos_rango(viejo, nuevo):
    url_api = "https://valorant-api.com/v1/competitivetiers"
    try:
        headers = {'User-Agent': 'Mozilla/5.0'}
        r = await asyncio.to_thread(requests.get, url_api, headers=headers, timeout=5)
        data = r.json()
        # Usamos el último juego de tiers competitivos oficiales vigentes
        tiers = data['data'][-1]['tiers']
        url_v, url_n = None, None
        for t in tiers:
            if t['tierName'].lower() == viejo.lower():
                url_v = t['largeIcon'] or t['smallIcon']
            if t['tierName'].lower() == nuevo.lower():
                url_n = t['largeIcon'] or t['smallIcon']
                
        bytes_v = await asyncio.to_thread(lambda: requests.get(url_v, headers=headers, timeout=5).content if url_v else None)
        bytes_n = await asyncio.to_thread(lambda: requests.get(url_n, headers=headers, timeout=5).content if url_n else None)
        return bytes_v, bytes_n
    except Exception as e:
        print(f"❌ Error obteniendo recursos gráficos de rangos: {e}")
        return None, None
...
def _vct_title(s): 
    try:
        return _cargar_fuente_proyecto("Anton-Regular.ttf", s)
    except OSError:
        return _bc_eb(s)
...
# ==============================================================================
# ENGINES GRÁFICOS COMPLEMENTARIOS
# ==============================================================================
...
def gen_gif_rank_up(titulo, viejo, nuevo, img_vieja_bytes, img_nueva_bytes, es_gran_subida):
    W, H = 880, 320
    frames, frame_durations = [], []
    ...
    for i in range(35):
        ...
        if i < 15:
            ...
            if img_n:
                img_n_scaled = img_n.resize((current_size, current_size), Image.Resampling.LANCZOS)
                if es_gran_subida:
...
async def _check_racha(nombre, tag):
    ...
    if not ultimas: return 0, None
    
    racha = 0
    tipo = "W" if ultimas[0]["won"] else "L"
    for r in ultimas:
        if ("W" if r["won"] else "L") == tipo:
            racha += 1
        else:
            break
    return racha, tipo
...
async def _check_rango(nombre, tag, nuevo_rango, canal):
    ...
    if viejo and viejo != nuevo_rango and nuevo_rango:
        ...
        if vi >= 0 and ni >= 0:
            if ni > vi:
                print(f"📈 Detectado ascenso para {nombre}#{tag}: {viejo} ➔ {nuevo_rango}")
                
                # Comprobamos si es un salto mayor de liga (ej: cambia la palabra de Silver a Gold)
                liga_vieja = viejo.split()[0].lower()
                liga_nueva = nuevo_rango.split()[0].lower()
                es_gran_subida = (liga_vieja != liga_nueva)
                
                # Descargamos los dos iconos en paralelo
                bytes_v, bytes_n = await _descargar_iconos_rango(viejo, nuevo_rango)
                
                # Generamos y enviamos el GIF animado
                tit = f"{nombre.upper()}#{tag.upper()}"
                buf = await asyncio.to_thread(gen_gif_rank_up, tit, viejo, nuevo_rango, bytes_v, bytes_n, es_gran_subida)
                
                await canal.send(file=discord.File(fp=buf, filename="rank_up.gif"))
            else:
                # Mantenemos tu alerta clásica estática de downgrade limpia
                buf = gen_banner_notificacion("📉 ¡DOWNGRADE DE RANGO!", f"{nombre}#{tag} ha caído de rango: {viejo} ➔ {nuevo_rango} 😬", _RED_G)
...
def generar_tarjeta(s, modo_display, tiene_datos_db, db_stats, top_agents_db, matches=None):
    ...
    if map_url:
        try:
            ...
            mapa_img = ImageEnhance.Color(mapa_img).enhance(0.55)
            img = mapa_img.copy()
            fondo_ok = True
        except:
            pass
...
# ==============================================================================
# SISTEMA INTERACTIVO DE BOTONES (UI VIEW) v1.0.4
# ==============================================================================
...
async def vigilante_partidas():
    ...
    try:
        ...
        print(f"⚠️ ERROR CRÍTICO: No se puede encontrar el canal de alertas (ID: {CANAL_ALERTAS_ID}). Error: {e}")
        return
        ...
        try:
            ...
            for lm in reversed(s["recent_matches"]):
                ...
                try:
                    ...
                    print(f"⚠️ Error procesando la partida {match_id} de {nombre}#{tag}: {e_inner}")
        
        # AQUÍ CERRAMOS EL TRY PRINCIPAL DE LA FUNCIÓN
        except Exception as e:
...
# ==============================================================================
# COMANDOS (SLASH COMMANDS REFACTORIZADOS)
# ==============================================================================
...
async def stats(interaction: discord.Interaction, nombre: str, tag: str, region: str = "eu", modo: app_commands.Choice[str] = None):
    ...
    rows = await bot.db.fetch(
        ...
        JOIN jugadores j ON p.jugador_nombre = j.nombre AND p.jugador_tag = j.tag
        WHERE j.server_id = $1
          AND p.jugador_nombre ILIKE $2
          AND p.jugador_tag ILIKE $3
    ...
    def _adr_row(r): return float(r["adr"] or 0)
    def _dda_row(r): return float(r["dda"] or 0)
    def _kast_row(r):
        if r["kast"] is not None: return float(r["kast"])
        return None
    ...
    try:
        ...
        # INSTANCIAMOS LA BOTONERA INTERACTIVA
        view_interactiva = StatsInteractiveHub(filtered_rows, s.get('nombre', nombre), s.get('tag', tag))
    except Exception as e:
    ...
    # MODIFICACIÓN: ELIMINADO EL EMBED, AHORA ENVÍA LA IMAGEN DIRECTAMENTE CON BOTONERA
    await interaction.followup.send(file=archivo, view=view_interactiva)
...
async def graficas(interaction: discord.Interaction, nombre: str, tag: str, modo: app_commands.Choice[str] = None):
    ...
    rows = await bot.db.fetch(
        ...
        FROM partidas p
        JOIN jugadores j ON p.jugador_nombre = j.nombre AND p.jugador_tag = j.tag
        WHERE j.server_id = $1 AND p.jugador_nombre ILIKE $2 AND p.jugador_tag ILIKE $3 AND ($4 = '%' OR LOWER(p.modo) = LOWER($4))
...
async def comparar(
    ...
    await interaction.followup.send(file=archivo)
...
async def temporada(interaction: discord.Interaction, modo: app_commands.Choice[str] = None):
    ...
    mvp = rows[0]
    mvp_txt = f"{mvp['nombre']}#{mvp['tag']}  (ACS Medio: {round(mvp['acs_medio'],1)} | WR: {round(mvp['winrate'],1)}%)"
    ...
    boxes = [
        ("MÁS PARTIDAS", f"{most_games['nombre']} ({most_games['partidas']})"), 
        ("MEJOR WINRATE", f"{best_wr['nombre']} ({round(float(best_wr['winrate']),1)}%)"), 
        ("MEJOR DDA", f"{best_dda['nombre']} ({round(float(best_dda['dda_medio']),1)})")
    ]
...
async def coach(interaction: discord.Interaction, nombre: str, tag: str):
    ...
    server_id = str(interaction.guild_id)
    ultima_partida = await bot.db.fetchrow(
        """
        SELECT p.kills, p.deaths, p.assists, p.acs, p.won, p.mapa, p.agente, p.hs, p.dda
        FROM partidas p
        JOIN jugadores j ON p.jugador_nombre = j.nombre AND p.jugador_tag = j.tag
        WHERE j.server_id = $1 AND p.jugador_nombre ILIKE $2 AND p.jugador_tag ILIKE $3
        ORDER BY p.fecha DESC
        LIMIT 1
        """,
        server_id, nombre, tag
    ...
    resultado = "Victoria" if ultima_partida["won"] else "Derrota"
    dda = float(ultima_partida["dda"] or 0)
