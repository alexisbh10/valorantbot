...
async def stats(interaction: discord.Interaction, nombre: str, tag: str, region: str = "eu", modo: app_commands.Choice[str] = None):
    ...
    # MODIFICACIÓN: ELIMINADO EL EMBED, AHORA ENVÍA LA IMAGEN DIRECTAMENTE
...
async def graficas(interaction: discord.Interaction, nombre: str, tag: str, modo: app_commands.Choice[str] = None):
    ...
    # MODIFICACIÓN: ELIMINADO EL EMBED DE TEXTO, SE ENVÍAN DIRECTAMENTE LAS IMÁGENES
...
async def comparar(
    ...
    # MODIFICACIÓN: ENVIADO COMO IMAGEN LIMPIA DIRECTA SIN CAJA EMBED
    await interaction.followup.send(file=archivo)
...
async def leaderboard(interaction: discord.Interaction, modo: app_commands.Choice[str] = None):
    ...
    # TRANSFORMACIÓN COMPLETA: LEADERBOARD AHORA SE DIBUJA COMO UNA TABLA EN IMAGEN PIL NATIVA
    filas_tabla = []
    for i, p in enumerate(scores):
        kda_val = round((p["tk"] + p["ta"]) / max(p["td"], 1), 2)
        # Añadir medallas directas de texto en la celda
...
async def temporada(interaction: discord.Interaction, modo: app_commands.Choice[str] = None):
    ...
    # TRANSFORMACIÓN COMPLETA: LA FICHA COMPLETA DE RESUMEN DE LA TEMPORADA SE GENERA COMO CANVAS NATIVO PIL
    mvp = rows[0]
